...
module.exports = yeoman.generators.Base.extend({
  constructor: function () {
    ...
    // Import our config from .yo-rc.json or bower.json
    if (fs.existsSync('.yo-rc.json')) {
      this.cpui = this.dest.readJSON('.yo-rc.json')['generator-cpui']
    }
    ...
    if (!this.cpui) {
      throw new Error("You must have a yeoman configuration in either: a .yo-rc.json file under a `generator-cpui` object, or in bower.json under a `yoconfig` object. Example: http://bit.ly/UZEQmW")
    }

    // Set defaults for where to poop things on disk
    this.cpui.paths = this.cpui.paths || {}
